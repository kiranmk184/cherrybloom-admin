...
function HoveredSubMenuItem({ icon, text, to, active }: SubMenuItemProps) {
    return (
        <NavLink
            to={to}
        ...
        >
            <div className="flex items-center justify-center ">
                <span className="text-primary-500 h-[2rem] w-[2rem] ">
                    {icon}
                </span>
                <span className="text-primary-500 ml-3 w-28 text-start">
                    {text}
                </span>
        ...
        <>
            <li>
                <NavLink
                    to={to}
                    className={`
                        group relative my-1 px-1 py-1 flex w-full cursor-pointer items-center rounded-[.5rem]
                        ${
                            !subMenu
                                ? `text-app-content ${
                                      !expanded && "bg-app-bg!"
                                  }`
                                : ` hover:text-app-content ${
                                      expanded && "hover:bg-app-content-15"
                                  }`
                        }
                        ${!expanded && "hidden sm:flex p-0"} text-app-content-85
                    `}
                    onClick={() =>
